...
For displaying tables and rooms a new tool is on the search as I would like it to be responsive and can meet my cretieria, at the moment due to school applications it is on delay. Measures against spamming reservsations will be implemented such as one table at a day per each account and in a real wolrd application it can be a charge if reservations are done and the person does not show up. User management will be handled by the django admin that is already included but I will implement one in the future as I would like t it to follow the design I have on the pages. 
